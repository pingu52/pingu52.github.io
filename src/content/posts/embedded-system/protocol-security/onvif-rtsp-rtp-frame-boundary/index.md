...
RTSP·SDP·패킷 예제는 설명용으로 새로 구성했습니다. 실제 장치 주소, 계정, 세션 식별자와 내부 경로는 사용하지 않았습니다. 아래 진단 명령을 해당 장비에서 새로 실행한 결과도 아닙니다.
:::
...
이후 별도 네트워크 환경에서는 해당 시험이 통과했다는 기록도 있습니다. 그렇다고 이전 실패의 원인이 특정 스위치나 설정이었다고 확정할 수는 없습니다. 두 실행의 조건이 충분히 통제됐는지, 무엇이 달라졌는지까지 확인해야 하기 때문입니다.
...
디버깅 메모에는 다음 세 가지를 함께 남기는 편이 좋습니다.
...
:::note
작성 환경의 FFmpeg 6.1.1에서 위 RTSP transport와 timeout 옵션이 제공됨을 확인했습니다. 실제 장치의 UDP·TCP·multicast 비교 결과는 이 글에서 새로 측정하지 않았습니다.
:::
...
RTSPS/TLS 또는 SRTP로 보호된 구간은 복호화 자료 없이 같은 수준으로 분석할 수 없습니다. 이 글의 평문 RTSP/RTP 필터 결과를 암호화된 스트림에 그대로 기대하지 않습니다.
:::
